...
| 2025-11-07 | `PYTHONPATH=.codex/vendor:src DATABASE_URL=sqlite:///tmp/full-test.db REDIS_ENABLED=0 PYTEST_ADDOPTS="-vv -s" python3 -m pytest tests/api/test_config_models.py` | ✅ 通过 | 惰性化 ModelGateway 后配置中心 API 用例恢复，通过自带桩即可运行 |
| 2025-11-07 | `PYTHONPATH=.codex/vendor:src python3 -m pytest tests/model_gateway/test_loader.py -q` | ✅ 通过 | 模型网关加载器单测验证记录转换与异常回退逻辑 |
| 2025-11-07 | `PYTHONPATH=.codex/vendor:src DATABASE_URL=sqlite:///tmp/full-test.db REDIS_ENABLED=0 python3 -m pytest tests/tasks/test_managed_cycle.py -q` | ✅ 通过 | 调度任务在动态加载网关设置后依旧通过，验证 Actor/Checker 初始化路径 |
| 2025-11-07 | `PYTHONPATH=.codex/vendor:src python3 -m pytest tests/model_gateway/test_service.py -q` | ✅ 通过 | ModelGateway 服务在缺省配置下会通过 loader 读取数据库记录，保障 CLI/脚本入口一致性 |
| 2025-11-07 | `PYTHONPATH=.codex/vendor:src DATABASE_URL=sqlite:///tmp/full-test.db REDIS_ENABLED=0 python3 -m pytest tests/api/test_monitoring.py -q` | ✅ 通过 | 新增 /api/monitor/llm-calls 审计接口验证，通过本地桩返回调用日志 |
| 2025-11-07 | `PYTHONPATH=.codex/vendor:src DATABASE_URL=sqlite:///tmp/full-test.db REDIS_ENABLED=0 python3 -m pytest tests/tasks/test_managed_cycle.py -q` | ✅ 通过 | `_ensure_decision_services` 在模型端点变化时重建 Actor/Checker/DecisionService，保持闭环 |
| 2025-11-07 | `PYTHONPATH=.codex/vendor:src DATABASE_URL=sqlite:///tmp/full-test.db REDIS_ENABLED=0 python3 -m pytest tests/trading/test_manager.py -q` | ✅ 通过 | RiskPolicy 阻断路径记录 RiskResult 与 DecisionLedger，验证执行闭环 |
| 2025-11-07 | `PYTHONPATH=.codex/vendor:src DATABASE_URL=sqlite:///tmp/full-test.db REDIS_ENABLED=0 python3 -m pytest tests/api/test_trading_decisions.py -q` | ✅ 通过 | 新增 /api/trading/decisions 审计接口单测，验证 DecisionLedger + RiskResult 输出 |
| 2025-11-07 | `PYTHONPATH=.codex/vendor:src DATABASE_URL=sqlite:///tmp/full-test.db REDIS_ENABLED=0 python3 -m pytest tests/api/test_monitoring.py tests/api/test_trading_decisions.py tests/tasks/test_managed_cycle.py tests/trading/test_manager.py -q` | ✅ 通过 | 监控/决策列表+详情 + Managed Cycle 刷新 + 执行闭环综合回归 |
