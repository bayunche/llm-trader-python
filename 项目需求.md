...
* **模型配置读写**
  输入字段（写入）：见“模型端点配置”表；通过 `PUT /api/config/models` 调用。
  输出字段：变更前后差异摘要、版本号、operator、applied_at；`GET /api/config/models` 返回当前生效的端点列表，`GET /api/config/models/metrics` 返回熔断计数、可用状态、last_error 等指标。
...
* **审计查询**
  ...
  输出字段：观测摘要、Actor/Checker/Risk 结论、订单与成交链路、模型调用摘要、费用与延迟指标；所有决策需存在 `DecisionLedger` 记录并关联 `LLMCallAudit` 与 `RiskResult`。
