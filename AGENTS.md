...
## 项目结构与模块划分
- **源代码**：位于 `src/llm_trader/`，按领域拆分（如 `data/`、`strategy/`、`trading/`、`reports/`）。执行适配相关逻辑在 `trading/execution_adapters.py`，端到端管线位于 `pipeline/auto.py`。
- **测试用例**：放置在 `tests/` 下，目录结构与源码对应（示例：`tests/trading/test_session.py`）。参考现有夹具与命名模式。
- **脚本与自动化**：`scripts/` 存放 CLI 入口（如 `run_ai_trading_cycle.py`）、全流程脚本 `run_full_pipeline.py`；容器相关文件在 `docker/`。
- **文档与配置**：设计文档在 `docs/`，环境模板 `.env.example`，Docker 配置 `docker-compose.prod.yml`、`Dockerfile`。

## 构建、测试与开发命令
- `poetry install` —— 安装全部 Python 依赖。
- `env PYTHONPATH=src python -m pytest` —— 运行单元与集成测试。
- `python scripts/run_full_pipeline.py` —— 本地执行数据同步→策略生成→交易执行→报表生成。
- `docker compose -f docker-compose.prod.yml up --build` —— 构建并启动容器，自动完成全流程并启动 Dashboard。

## 编码风格与命名约定
- 遵循 **PEP 8**，四空格缩进；模块、函数、变量使用蛇形命名，类使用帕斯卡命名。
- 注释统一使用简体中文，保持语句简洁。
- 优先使用 pathlib；导入顺序为标准库→第三方→本地模块。
- 使用类型注解，新文件建议加入 `from __future__ import annotations`。

## 测试准则
- 测试框架为 **pytest**，公共夹具位于 `tests/conftest.py`。测试文件命名与源代码对应。
- 测试函数命名采用 `test_<行为>`，覆盖沙盒/实盘模式等关键分支。
- 当无法安装依赖时，可使用 `python3 -m compileall src` 进行语法检查。

## 提交与 Pull Request 规范
- Commit 信息使用祈使句现在时（示例：`Add execution adapter`）。保持一次提交只处理一类变更。
- PR 需包含：变更摘要、测试说明（命令或 compileall 结果）、必要的截图或日志、关联的任务或 Issue。
- 若改动影响行为或流程，请同步更新 `README.md`、`verification.md`、`.codex/operations-log.md`。

## 安全与配置提示
- 所有密钥通过环境变量传入；勿提交凭据。`.env.example` 罗列了必要项（如 `OPENAI_API_KEY`）。
- 默认使用沙盒模式（`TRADING_EXECUTION_MODE=sandbox`）；切换至 `live` 前必须设置券商适配器并完成额外审核。

## 补充规范

- 永远用中文交流与文档。

- 只做被要求的事；优先修改现有文件；无明确指令不新建文件。

- 每次修改：lint → build → test；失败则回滚并给替代方案。

- 严禁编造 API/库函数；上下文不足必须先提问。

- 每次工作必须生成详细的 Markdown 步骤文档，可追踪、可复现、可回滚

- **使用简体中文思考和对话**，所有注释与文档也用中文。
- **小步修改，逐步提交**：先解释方案，再输出代码补丁。
- **不编造**：API、依赖、参数若不确定，必须先提问确认。
- **最小变更**：优先编辑现有文件，而不是新建。
- **兼顾安全、性能、可维护性**。
...
### 自检清单
 ...
 构建通过：lint/build/test 全部通过

 性能合理：无明显退化，说明差异

 安全合规：输入校验、防注入/XSS/CSRF

 一致性：命名、风格、目录符合规范

 可维护性：单一职责，无重复，必要注释

 文档完整性：分步骤 Markdown 已生成

 回归验证：列出受影响模块 + 验证方法

 回滚策略：说明 revert/还原补丁方法

### 常见澄清问题

是否允许新增依赖？体积/性能预算？

是否需要兼容旧版本（Node/浏览器/移动端）？
...
### Code Quality
- 每次提交：可编译 + 全部测试通过 + 新功能有测试 + 格式规范。
- 提交前：运行 lint/format、自审、写清楚 commit message。
